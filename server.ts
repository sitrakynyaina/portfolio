...
// System instructions for Marcus Vance's AI twin
const MARCUS_RESUME_PROMPT = `
You are the interactive AI Twin & Professional Advisor of Marcus Vance. Your goal is to represent him to hiring managers, technical recruiters, and other developers visiting his portfolio.
...
Key Facts about Marcus Vance:
- Title: Senior Full-Stack Developer & Systems Architect
- Experience: Over 7 years of industrial experience designing and implementing high-throughput web applications and scalable systems.
...
- Core Technologies:
  - Frontend: TypeScript, React 19, Next.js, Tailwind CSS, Recharts, Framer Motion
  - Backend: Node.js, Go, Express, gRPC, Apache Kafka
  - Databases: PostgreSQL, Redis, MongoDB, Elasticsearch
  - Cloud / DevOps: Kubernetes, Docker, Google Cloud (Cloud Run, GKE, Pub/Sub), Terraform, CI/CD pipelines
- Projects Highlights:
...
- Answer technical questions about Marcus's skills, experience, or project details accurately based on the facts above.
- If asked about compensation, tell them to get in touch via the Contact section or email landrysitrak@gmail.com.
- If asked about hypothetical complex tasks (e.g., "Could Marcus build an AI scraper with Redis caching?"), answer enthusiastically yet realistically, outlining Marcus's technical strategy using Redis queues, Go concurrency, and node workers.
- Always refer to yourself as "Marcus's AI Assistant" or "Marcus's Career Advisor" and address Marcus in the third-person or respond as his dedicated agent.
`;
...
app.post("/api/career-assistant", async (req:any, res:any) => {
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      contents: contents,
      config: {
        systemInstruction: MARCUS_RESUME_PROMPT,
        temperature: 0.7,
      },
    });
...
function generateMockResponse(prompt: string): string {
  ...
  if (query.includes("skill") || query.includes("tech") || query.includes("language")) {
    return "Marcus specializes in TypeScript, React, Node.js, Go, PostgreSQL, Redis, Kubernetes, and Google Cloud Platform. He is highly proficient in architectural scalability.";
  }
  if (query.includes("contact") || query.includes("hire") || query.includes("email")) {
    return "You can contact Marcus directly by submitting the form on the Contact section or emailing him at landrysitrak@gmail.com.";
  }
  if (query.includes("project") || query.includes("work")) {
    return "Marcus's portfolio features three flagship projects: the SaaS Enterprise Financial Dashboard, the Apex Mesh Cloud Topology Explorer, and the Synapse Telemetry Broker. Let me know if you want deep details on any of these.";
  }
  return "Hello! I am Marcus Vance's AI career advisor. Ask me anything about his engineering experience, technical stack, or projects. (Since no GEMINI_API_KEY is currently provided in early preview, I am running on fallback mode!)";
}
